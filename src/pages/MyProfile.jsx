...
const MyProfile = () => {
    const { token, setToken, backendUrl, navigate, currency } = useContext(ShopContext);
                ...
                <div className='w-full md:w-3/4'>
                    {activeTab === 'orders' ? (
                                ...
                                <div key={i} className='border rounded-xl p-6 bg-white shadow-sm'>
                                    <div className='flex justify-between items-center border-b pb-4 mb-4'>
                                        ...
                                        <div className='text-right'>
                                            <p className={`font-bold ${order.status === 'Delivered' ? 'text-green-600' : 'text-blue-600'}`}>{order.status}</p>
                                    ...
                                    <div className='flex justify-between items-center pt-4 border-t bg-gray-50 -mx-6 -mb-6 px-6 py-3 rounded-b-xl'>
                                        <span className='font-bold text-gray-600'>Amount Due (COD):</span>
                                        <span className='text-xl font-bold text-red-500'>{currency}{order.amount}</span>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className='bg-white border rounded-xl p-8'>
                            ...
                            <button className='mt-6 px-6 py-2 border border-blue-500 text-blue-500 rounded-full hover:bg-blue-50' onClick={()=>toast.info("Edit feature coming next update!")}>Edit Profile</button>
                        </div>
                    )}
                </div>
            </div>
